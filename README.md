...
1. Перейдите в директорию в которой хранятся скрипты

```
cd <директория, в которой хранятся скрипты> (например: cd /root/backup/)
```
2. Запуск скрипта, который создает полный бэкап (если он еще не был сегодня создан)

```
chmod u+x full-backup.sh
./full-backup.sh
```
3. Запуск скрипта, который создает первичный или дополнительный инкрементный бэкап (в зависимости от того создали вы уже сегодня инкрементный бэкап или нет)

```
chmod u+x diff-backup.sh
./diff-backup.sh
```
4. Запуск скрипта, который применяет и подготавливает все сегодняшние инкрементные бэкапы к полному бэкапу
...
5. Запуск скрипта, который восстанавливает последний полный бэкап или полный бэкап + инкрементный бэкап с заменой исходной базы (операция необратима)
